...
js_code = re.sub(r"\s*// Reabrir modal de venta.*?setTimeout.*?\}, 300\);", "", js_code, flags=re.DOTALL)
...
# HTML insertion right before <script> tags
html_target = '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>'
injection_point_html = registrar_content.find(html_target)
if injection_point_html == -1:
    raise Exception("Could not find script insertion point")
...
# JS insertion right inside the <script> block
js_target = 'const TIPOS_PAGO_TEMPLATE'
