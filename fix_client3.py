...
modal_start = ventas_content.find('<div class="modal fade" id="modalAgregarClienteVenta"')
modal_end = ventas_content.find('<!-- Modal Seleccionar Servicio / Trámite -->', modal_start)

if modal_start == -1 or modal_end == -1:
    raise Exception("Could not find modal HTML boundaries in ventas.html")
...
if js_start == -1 or js_end == -1:
    raise Exception("Could not find JS boundaries in ventas.html")
