...
# Find line 1515 in ventas.html which is the end of the conyuge logic
# It's right before "// ⭐ INICIALIZAR DETALLES VENTA" or something similar.
js_end = ventas_content.find("// ⭐ FUNCIÓN PARA AGREGAR FILA DE PRODUCTO", js_start)
...
# Wait, `cerrarModalClientePreCredito` re-opens `modalNuevaVenta`. We should remove that behavior!
# Let's just strip out the re-opening modal part.
js_code = re.sub(r"// Reabrir modal de venta después de un momento.*?setTimeout\(\(\) => \{.*?modalVenta\.show\(\);\s*\}, 300\);", "", js_code, flags=re.DOTALL)
